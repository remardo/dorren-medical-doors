...
if [ -f "/etc/letsencrypt/live/$DOMAIN/fullchain.pem" ]; then
    echo "✅ Сертификат найден в live директории"

    # Продолжение выполнения скрипта для live сертификата
    # Шаг 2: Создание правильной конфигурации
    echo "⚙️ Шаг 2: Создание конфигурации nginx..."
    echo "✅ Сертификат найден в renewal конфигурации"
    echo "Проверка статуса сертификата..."
    sudo certbot certificates

    # Попытка создать символические ссылки
    echo "🔗 Создание символических ссылок..."
    sudo certbot install --cert-name $DOMAIN 2>/dev/null || echo "Сертификат уже установлен"
